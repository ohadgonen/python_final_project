...
def plot_brain_activity_by_disorder_real(df, main_disorder):
    ...
    Returns:
        None: Displays a single figure with subplots for all frequency bands.
    """
    # Valid frequency bands (add or modify based on your dataset)
    valid_bands = ["delta", "theta", "alpha", "beta", "gamma", "highbeta"]

    # Set up the figure and subplots
    fig, axes = plt.subplots(3, 2, figsize=(15, 18))  # Increased vertical size
    axes = axes.flatten()  # Flatten to easily iterate over subplots

    for i, frequency_band in enumerate(valid_bands):
        ax = axes[i]

        # Filter dataset by the main disorder
        filtered_df = df[df['main.disorder'] == main_disorder]
        band_columns = [col for col in df.columns if col.startswith(frequency_band)]

        if not band_columns:
            ax.set_visible(False)  # Hide subplot if no data for this band
            continue

        # Group by specific disorder and calculate the mean for the frequency band
        mean_activity = (
            filtered_df.groupby('specific.disorder')[band_columns]
            .mean()
            .mean(axis=1)
